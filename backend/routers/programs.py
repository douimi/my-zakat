...
async def upload_program_video(
    ...
    if program.video_filename:
        if program.video_filename.startswith('http://') or program.video_filename.startswith('https://'):
            object_key = extract_object_key_from_url(program.video_filename)
            if object_key:
    ...
    try:
        ...
        s3_url = upload_file(
            ...
            metadata={"original_filename": file.filename or "", "type": "program_video", "program_id": str(program_id)}
        )
